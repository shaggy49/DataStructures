 ...
 * It returns a valid pointer on success, NULL on error. 
 */
UnsortedArray *array_to_sort_create(int (*compare)(void *, void *));

/* 
 * It accepts as input a pointer to an unsorted array and
 * it returns 1 iff the unsorted array is empty (0 otherwise), -1 on error.
 * The input parameter cannot be NULL
 */
 ...
 * it returns the pointer to the i-th element of the unsorted array, NULL on error.
 ...
 * It returns 0 on success, -1 on error.
 */
int array_to_sort_free_memory(UnsortedArray *);
 ...
 * It returns 0 on success, -1 on error.
 */
int array_to_sort_merge_binary_insertion_sort(UnsortedArray *, unsigned long, unsigned long);
