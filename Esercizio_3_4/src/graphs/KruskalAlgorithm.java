...
/* prende in input un grafo non orientato e connesso e restituisce una foresta (lista di archi) --- kruskMST (minimo albero ricoprente) */

public class KruskalAlgorithm <V,E extends Comparable<E>> {
    public  Graph<V,E> kruskalAlgorithm(Graph<V,E> gr) {
        Graph <V,E> kruskMST = new Graph<>("undirected");
        UnionFindSet<V> grSet = new UnionFindSet<>();				//mettiamo qui tutti i set creati a riga 17
        List<Edge<V,E>> listEdges = new LinkedList<>();
		try{
        ...
        for(Edge<V,E> grEdge : listEdges){
			...
			if (!(firstNode.equals(secondNode))){
				kruskMST.addEdge(grEdge.getStartNode(), grEdge.getEndNode(), grEdge.getEdgeWeight());
		...
		/* 
		long nOfNodes = gr.getNumberOfNodes();
        // Index used to pick next edge
        long i = 0; 
        // An index variable, used for kruskMST
        long e = 0;
        // Number of edges to be taken is equal to V-1
		while (e < nOfNodes - 1){
			// Step 2: Pick the smallest edge. And increment
			// the index for next iteration
			Edge<V,E> nextGraphEdges = gr.graphEdges().get(i++);
			V x = find(nextGraphEdges.getStartNode());
			V y = find(nextGraphEdges.getEndNode());
			// If including this edge does't cause cycle,
			// include it in result and increment the index
			// of result for next edge
			if (x != y) {
                kruskMST.add(e++, nextGraphEdges);
                grSet.union(x,y);
			}
        }
        return kruskMST; */
...
class Graph {
	...
	void KruskalMST()
	{
        ...
        //corrispettivo di unionFindeSet
